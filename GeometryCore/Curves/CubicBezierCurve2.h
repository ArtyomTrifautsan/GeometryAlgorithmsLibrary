...
namespace Geometry
{
	...
	struct CubicBezierCurve
	{
		...
		constexpr CubicBezierCurve() = default;
		constexpr CubicBezierCurve(Point2<T> P0, Point2<T> P1, Point2<T> P2, Point2<T> P3) noexcept
		...
		constexpr std::pair<CubicBezierCurve, CubicBezierCurve> Subdivide(T t) const noexcept
		{
			const Point2<T> q0 = p0 + t * (p1 - p0);
			const Point2<T> q1 = p1 + t * (p2 - p1);
			const Point2<T> q2 = p2 + t * (p3 - p2);
			...
			return {
				CubicBezierCurve{ p0, q0, r0, s0 },
				CubicBezierCurve{ s0, r1, q2, p3 }
			};
		}
	};
}